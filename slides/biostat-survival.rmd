...
```{r km_censored, fig.cap = 'KM - censored', ref.label='km_basic', warning=F, echo=F, message=F}
```
...
```{r km_best_worst, fig.cap = 'KM - actual/best/worst', ref.label='km_basic', warning=F, echo=F, message=F}
```
...
```{r km_scenario, fig.cap = 'KM - scenarios', ref.label='km_basic', warning=F, echo=F, message=F}
```
...
```{r km_ci, fig.cap = 'KM - CI', ref.label='km_basic', warning=F, echo=F, message=F}
```
...
在理论上，生存函数是平滑的；在实践中，我们在离散时间尺度上观察事件。
