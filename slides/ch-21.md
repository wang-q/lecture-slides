...
\begin{columns}[onlytextwidth]
  \column{0.5\textwidth}
    \begin{figure}
      \includegraphics{ch-21.images/image5.jpeg}
      \caption{结构示意图}
    \end{figure}

  \column{0.5\textwidth}
      \begin{figure}
        \includegraphics{ch-21.images/image4.png}
        \caption{透射电镜照片}
      \end{figure}
\end{columns}
...
* Watson和Crick.
* DNA分子由两条互补核苷酸单链组成; 两条核苷酸链总是按碱基A与T, G与C互补配对, 成反向平行, 通过氢键形成稳定的双螺旋结构.
...
![DNA结晶的X-射线衍射照片](ch-21.images/image12.png)
...
![DNA双螺旋结构2](ch-21.images/image19.jpeg)
...
\begin{tikzpicture}[node distance=1cm, auto,]
  %nodes
  \node[punkt] (0) {前体};
  \node[punkt, right=1.5cm of 0] (1) {鸟氨酸};
  \node[punkt, right=1.5cm of 1] (2) {瓜氨酸};
  \node[punkt, right=1.5cm of 2] (3) {精氨酸};

  \path
    (0) edge [pil, right] node[auto] {酶I(\textit{arg1})} (1)
    (1) edge [pil, right] node[auto] {酶II(\textit{arg2})} (2)
    (2) edge [pil, right] node[auto] {酶III(\textit{arg3})} (3);
\end{tikzpicture}
...
1. RNA的结构与功能
    * 与DNA相比较，其结构特点：单链、戊糖是核糖、尿嘧啶代替胸腺嘧啶。
    * 功能：信使RNA、核糖体RNA、转运RNA。
...
![阅读框及其重要性](ch-21.images/image34.jpeg)
...
\begin{tikzpicture}[node distance=1cm, auto,]
  %nodes
  \node[punkt] (dna) {DNA};
  \node[punkt, right=0.8cm of dna] (rna) {RNA};
  \node[punkt, right=0.8cm of rna] (pro) {PROTEIN};

  \path
    (dna) edge [pil, loop left] node {} (dna)
          edge [pil, right] node {} (rna)
    (rna) edge [pil, right] node {} (pro);
\end{tikzpicture}
